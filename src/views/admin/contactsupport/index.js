...
// export default CustomersAndSupport;
  ...
  const UserItem = styled(ListItem)(({ theme, selected }) => ({
    cursor: "pointer",
    backgroundColor: selected ? "#11047A !important" : "transparent",
    color: selected ? theme.palette.primary.contrastText : theme.palette.text.primary,
    "&:hover": {
      backgroundColor: "#11047A",
      color: theme.palette.primary.contrastText,
  ...
  const CustomersAndSupport = () => {
    ...
    const handleUserSelect = (user) => {
      ...
      getChatMessages(user) // Change to use _id for the customer ID
        .then((response) => {
          // Handle the response here
          console.log("Chat messages for", user.customerName, ":", response.data);
          // Update chatMessages state with the fetched messages
          setChatMessages(response.data.data);
        })
        .catch((error) => {
          // Handle errors
    ...
    // const handleDeleteMessage = (messageId) => {
    //   // Call the necessary API to delete the message
    //   handleDeleteConfirmation(messageId);
                    ...
                    <List>
                      {chatMessages.map((chat, index) => (
                        ...
                        >
                          ...
                          <ChatMessageText
                            primary={chat.isCustomerSender ? "You" : "Admin"}
                            secondary={chat.message}
